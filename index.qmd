...
<!--# Environmental variables were screened for multicollinearity and to identify redundant descriptors. Variables representing similar environmental information were also combined when appropriate, thereby reducing redundancy in the dataset [@legendre1998]. These data were then analyzed using Principal Component Analysis (PCA) to summarize multivariate patterns of environmental variation among sampling sites. Prior to PCA, site morphology and water-quality variables were square-root transformed. Variables describing sediment composition and marginal habitat structure, expressed as percentages, were relativized by their column totals and subsequently subjected to the arcsine square-root transformation [@mccune2002]. PCA was conducted in R using the `FactoMineR` package [@lê2008], with all variables centered and standardized to unit variance before analysis.-->
...
habitat <- read.xlsx("dados/rebio23-habitat.xlsx",
                    ...
                    colNames = T,
                    sheet = "ambiente")
grupos <- read.xlsx("dados/rebio23-habitat.xlsx",
                   rowNames = T,
                   colNames = T,
                   sheet = "grupos")
...
library(openxlsx)
zoo <- read.xlsx("C:/Users/ellen/Downloads/Manuscritos/rebio23-part/dados/rebio23-zoopl.xlsx",
                       rowNames = T,
                       colNames = T,
                       sheet = "rebio23_80ml")
#View(zoo)
...
<!--# ####UNIVARIADAS#### Zooplankton diversity was characterized using species richness (R) and Shannon's density (D) [@magurran2013]. Species richness was calculated as the number of species recorded in each sample, whereas density was expressed as the number of individuals per cubic meter (ind m⁻³) [@bowen2017]. Variation in richness and density was evaluated using factorial analysis of variance (ANOVA). Each observation was classified according to three factors: river reach (three levels), habitat (three levels), and hydrological season (wet and dry). The analysis initially considered the full factorial model. When significant interactions were detected, higher-order interactions were examined first, followed by the corresponding two-way interactions (reach × season, reach × habitat, and season × habitat) and, subsequently, the main effects. Response variables were transformed using log₁₀(x + 1) to improve compliance with assumptions of normality and homogeneity of variances [39]. Differences among factor levels were subsequently investigated using Tukey's HSD test at α = 0.05. -->

<!--# All data analysis is based on the density, number of individuals per liter (ind./L), averaged among the three subsamples for each site (Bowen 2017, Silva 2017). Sites were compared in single units or as groups in accordance with their location, stream inside the REBIO (Barro Branco stream fixed sites), stream outside the REBIO (Caiana stream fixed sites) and the surrounding sites. Densities and richness of species were compared among study sites using one-way ANOVA followed by post hoc multiple comparisons using Tukey’s HSD test (α=0.05) (Zar 1999). Density was square root transformed, and the environmental variables were log10 (x+1) transformed to enhance normality and homogeneity of variances (Sokal e Rohlf 1995). To describe the univariate correlations between density and richness with the environmental variables a stepwise multiple regression (SMR) with forward selection was performed, where variables are entered in the model based on the significance (probability) of the F value (Sheridan and Lyndall 2001, Maltchik et al. 2010).-->

<!--# ####CLASSIFICAÇÃO#### Differences in assemblage composition across the spatial hierarchy were explored using hierarchical agglomerative clustering. Bray–Curtis dissimilarities were calculated from the taxa density matrix, and clusters were formed using the flexible beta linkage method (β = −0.25) (@Ávila2011).Prior to the analyses, density data were relativized and square-root transformed to reduce the influence of highly abundant taxa and improve the distributional properties of the dataset (@Rocha2012). The resulting dendrogram was interpreted visually, and the number of groups retained was determined using a cut-off corresponding to 25% or less of the remaining information (@Quinn2002). The statistical significance of compositional differences among groups and spatial levels was evaluated using the Multi-Response Permutation Procedure (MRPP) (@Biondini1985; @McCuneGrace2002). In addition to the probability associated with the test, the MRPP agreement statistic (*A*) was used to describe within-group homogeneity relative to that expected under random grouping. Both the hierarchical clustering and MRPP analyses were conducted in PC-ORD version 7.07 (@McCuneMefford2018).-->

<!--# ####PARTIÇÃO#### Additive diversity partitioning was used to quantify the contribution of within- and between-unit diversity to total zooplankton diversity. In this framework, alpha diversity (α) represents diversity occurring within sampling units, whereas beta diversity (β) represents the contribution of differences among units at successive levels of the spatial hierarchy. The hierarchical structure included three spatial scales. The finest scale consisted of individual 1-mL subsamples obtained from each zooplankton sample (n = 27), allowing variation within samples to be assessed. The intermediate scale corresponded to habitats within each river reach (n = 9), whereas the broadest scale represented differences among the three river reaches (n = 3). Accordingly, total diversity (γ) was decomposed into within-subsample diversity (α₁) and the successive beta components associated with differences among subsamples (β₁), among habitats within river reaches (β₂), and among river reaches (β₃). The contribution of each component was expressed as a percentage of total diversity. To investigate seasonal variation in the hierarchical structure of diversity, additive partitioning was performed independently for the wet and dry seasons. Analyses were conducted in PARTITION 3.0 [40] using individual-based randomizations under a Type I null model [41]. For each analysis, 10,000 randomizations were performed to generate null distributions for alpha and beta diversity at each hierarchical level. The probabilities generated by the randomization procedure were interpreted as two-tailed departures from the null expectation: values below 0.05 indicated diversity components greater than expected by chance, whereas values above 0.95 indicated components significantly lower than expected [42]. -->

<!--# ####INTERPRETAÇÃO#### Additive diversity partitioning was conducted in R using the `adipart` function implemented in the **vegan** package (@Oksanen2017; @RCoreTeam2013). Significance was evaluated using individual-based randomizations under a Type I null model (@Crist2003). For each hierarchical level, 10,000 randomized communities were generated to produce null distributions for the corresponding α- and β-diversity components. The observed diversity components were then compared with their respective null distributions. The probabilities generated by the randomization procedure were interpreted as measures of departure from random expectations: values below 0.05 indicated that the observed component was significantly greater than expected under the null model, whereas values above 0.95 indicated that the observed value was significantly lower than expected (@Gering2003; @Veech2003; @VeechCrist2009).-->
